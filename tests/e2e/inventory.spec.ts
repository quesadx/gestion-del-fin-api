...
test.describe('GET /api/inventory/:campId', () => {
  test('returns camp inventory for admin', async ({ adminRequest }) => {
    const res = await adminRequest.get('/inventory/1');
  ...
  test('returns 401 when unauthenticated', async () => {
    ...
    const res = await ctx.get('/inventory/1');
...
test.describe('GET /api/inventory/audit/:campId', () => {
  test('returns inventory audit log', async ({ adminRequest }) => {
    const res = await adminRequest.get('/inventory/audit/1');
    expect(res.ok()).toBeTruthy();
    const body = await res.json();
    expect(body).toHaveProperty('data');
  });
  ...
  test('returns 401 when unauthenticated', async () => {
    ...
    const res = await ctx.get('/inventory/audit/1');
    await expectError(res, 401);
    await ctx.dispose();
  });
});
...
test.describe('POST /api/inventory/adjustment', () => {
  test('adjusts inventory and reflects change', async ({ adminRequest }) => {
    const listRes = await adminRequest.get('/resources');
    const listData = await listRes.json();
    const resourceId = listData.data[0].id;

    const adj = await adminRequest.post('/inventory/adjustment', {
      data: {
        camp_id: 1,
    ...
    const inv = await adminRequest.get('/inventory/1');
    const body = await inv.json();
    expect(body.data).toBeTruthy();
  });

  test('returns 400 when quantity is missing', async ({ adminRequest }) => {
    const res = await adminRequest.post('/inventory/adjustment', {
      data: {
        camp_id: 1,
        resource_type_id: 1,
        type: 'MANUAL_IN',
  ...
  test('returns 401 when unauthenticated', async () => {
    ...
    const res = await ctx.post('/inventory/adjustment', {
      data: {
        camp_id: 1,
        resource_type_id: 1,
        type: 'MANUAL_IN',
...
test.describe('Cross-camp inventory isolation', () => {
  ...
  test('camp 2 inventory exists separately', async ({ adminRequest }) => {
    const res = await adminRequest.get('/inventory/2');
    expect(res.ok()).toBeTruthy();
  });

  test('worker_camp1 can see camp 1 inventory', async ({ workerCamp1Request }) => {
    const res = await workerCamp1Request.get('/inventory/1');
    expect(res.ok()).toBeTruthy();
  });
});
