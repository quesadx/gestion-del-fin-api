...
const (
	...
	jobQueueCleanupExpiredTokens = "jobs:cleanup_expired_tokens"
...
func main() {
	...
	if valkeyURL != "" {
		log.Printf("Starting worker in daemon mode (queues: %s, %s, %s, %s)", jobQueueDailyRations, jobQueueDailyProduction, jobQueueResourceAlerts, jobQueueCleanupExpiredTokens)
...
func runOnce(ctx context.Context, pool *pgxpool.Pool, childAge int) error {
	...
	if err := processCleanupExpiredTokens(ctx, pool); err != nil {
		log.Printf("[JOB] Cleanup expired tokens error: %v", err)
	}
...
func runDaemon(ctx context.Context, pool *pgxpool.Pool, redisUrl string, childAge int) error {
	...
	queues := []string{jobQueueDailyRations, jobQueueDailyProduction, jobQueueResourceAlerts, jobQueueCleanupExpiredTokens}
	...
	for {
		...
		case "cleanup_expired_tokens":
			errProc = processCleanupExpiredTokens(ctx, pool)
		default:
...
func processCleanupExpiredTokens(ctx context.Context, pool *pgxpool.Pool) error {
	result, err := pool.Exec(ctx, `DELETE FROM refresh_tokens WHERE expires_at < NOW() OR revoked_at IS NOT NULL`)
	if err != nil {
		return fmt.Errorf("failed to clean up expired/revoked refresh tokens: %w", err)
	}
	n := result.RowsAffected()
	log.Printf("[JOB] Cleaned up %d expired/revoked refresh token(s)", n)
	return nil
}
...
func queueForJobType(jobType string) string {
	...
	case "cleanup_expired_tokens":
		return jobQueueCleanupExpiredTokens
	default:
		return ""
	}
}
...
func logLowResourceAlerts(ctx context.Context, pool *pgxpool.Pool, campId int) error {
	...
	for rows.Next() {
		var qty, min float64
		var name string
		if err := rows.Scan(&qty, &min, &name); err != nil {
			return err
		}
		status := "OK"
		if qty < min*0.5 {
