...
const personInclude = { camps: true, professions: true } as const;

function parseAdmittedAt(dateStr?: string) {
...
async function ensureProfessionExists(professionId: number) {
  ...
  if (!profession) throw new AppError(`profession_id does not exist: ${professionId}`, 404);
}
...
async function validateRelations(campId?: number, professionId?: number) {
  if (campId !== undefined) {
    await ensureCampExists(campId);
  }

  if (professionId !== undefined) {
    await ensureProfessionExists(professionId);
  }
}
...
function preparePersonCreateData(data: CreatePersonDto) {
  return {
    ...
    admitted_at: parseAdmittedAt(data.admitted_at)!,
...
function preparePersonUpdateData(data: UpdatePersonDto) {
  ...
  return {
    ...
    admitted_at: admitted_at ? parseAdmittedAt(admitted_at) : undefined,
...
export async function createPerson(data: CreatePersonDto) {
  await validateRelations(data.camp_id, data.profession_id);
...
export async function getPerson(id: number) {
  const result = await prisma.persons.findUnique({
    where: { id },
    include: personInclude,
  });

  if (!result) throw new AppError(`Person not found: ${id}`, 404);
