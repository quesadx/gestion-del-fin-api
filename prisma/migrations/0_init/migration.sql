-- ============================================================
-- Grants (from 00-grants.sql) 
-- Enables Prisma to create/drop for shadow database in dev
-- ============================================================
GRANT CREATE, DROP ON *.* TO 'admin'@'%';
FLUSH PRIVILEGES;

-- ============================================================
-- Schema & Tables (from 01-gestion-del-fin-seed.sql)
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `gestion_del_fin`.`system_config` (
  ...
  PRIMARY KEY (`id`)
) ENGINE = InnoDB COMMENT = 'General system parameters';
...
CREATE TABLE IF NOT EXISTS `gestion_del_fin`.`roles` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `name_UNIQUE` (`name` ASC)
) ENGINE = InnoDB COMMENT = 'Roles that can be assigned to users';
...
CREATE TABLE IF NOT EXISTS `gestion_del_fin`.`users` (
  ...
  CONSTRAINT `fk_role_id`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
) ENGINE = InnoDB COMMENT = 'Survivors';
...
CREATE TABLE IF NOT EXISTS `gestion_del_fin`.`persons` (
  ...
  CONSTRAINT `fk_persons_profession_id`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
) ENGINE = InnoDB COMMENT = 'Also known as survivors';
...
CREATE TABLE IF NOT EXISTS `gestion_del_fin`.`admission_requests` (
  ...
  `id_card_url` VARCHAR(500) NULL COMMENT 'Optional image of applicant identification card',
...
CREATE TABLE IF NOT EXISTS `gestion_del_fin`.`contribution_overrides` (
  ...
  CONSTRAINT `fk_created_by`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
) ENGINE = InnoDB COMMENT = 'Overrides profession-defined contribution amounts';
...
CREATE TABLE IF NOT EXISTS `gestion_del_fin`.`camp_transfers` (
  ...
  `leader_person_id` INT UNSIGNED NULL COMMENT 'Physical leader of transfer (person, not user)',
  `scheduled_delivery_date` DATETIME NULL COMMENT 'Scheduled delivery/return date',
  `created_by` INT UNSIGNED NOT NULL,
  `approved_source_by` INT UNSIGNED NULL,
  `approved_target_by` INT UNSIGNED NULL,
  ...
  `created_at` DATETIME NOT NULL DEFAULT NOW(),
  PRIMARY KEY (`id`),
  INDEX `fk_camp_transfers_requesting_camp_idx` (`requesting_camp` ASC),
  INDEX `fk_camp_transfers_target_camp_idx` (`target_camp` ASC),
  INDEX `fk_leader_person_id_idx` (`leader_person_id` ASC),
  INDEX `fk_created_by_idx` (`created_by` ASC),
  CONSTRAINT `fk_camp_transfers_requesting_camp`
    FOREIGN KEY (`requesting_camp`)
    REFERENCES `gestion_del_fin`.`camps` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_camp_transfers_target_camp`
    FOREIGN KEY (`target_camp`)
    REFERENCES `gestion_del_fin`.`camps` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_leader_person_id`
    FOREIGN KEY (`leader_person_id`)
    REFERENCES `gestion_del_fin`.`persons` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_camp_transfers_created_by`
    FOREIGN KEY (`created_by`)
    REFERENCES `gestion_del_fin`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION
